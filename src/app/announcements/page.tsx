    ...
    try {
      ...
      if (isSameDay) {
        ...
        try {
          ...
          const symbolToUse = d.symbol || scripCode
          const hRes = await fetch(`/api/bse/history?symbol=${encodeURIComponent(symbolToUse)}&scripCode=${scripCode}&fromDate=${fetchStart.toISOString().split('T')[0]}&toDate=${fetchEnd.toISOString().split('T')[0]}`)
          if (hRes.ok) {
            ...
            if (hData.data && hData.data.length > 0) {
              ...
              if (closest && closest.close) {
                ...
                announcementPricesRef.current.set(announcementId, closest.close)
                try {
                  const stored = JSON.parse(localStorage.getItem('speedy_announcement_prices') || '{}')
                  stored[announcementId] = closest.close
          ...
          console.warn("Failed to fetch historical price, falling back to previousClose", e)
        }
