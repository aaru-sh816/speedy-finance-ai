...
import { getAnnualPriceRatios, resolveNseSymbol } from "@/lib/finedge"
...
export async function GET(
  ...
  try {
    ...
    const data = await getAnnualPriceRatios(nseSymbol, { statement_type })
    ...
    console.error("[FinEdge annual-price-ratios]", e)
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Failed to fetch annual price ratios" },
      { status: 500 }
    )
  }
}
