...
export function LightweightChart({
  ...
  useEffect(() => {
    ...
    chart.subscribeCrosshairMove(param => {
      ...
      if (
        ...
        if (onCrosshairMove) {
          onCrosshairMove(price ?? null, param.time ? String(param.time) : null)
        }
  ...
  useEffect(() => {
    ...
    if (dataPoint) {
      ...
      const timer = setTimeout(() => {
        ...
        if (x !== null && y !== null) {
          const whaleDeal = aggregatedDeals.find(w => w.date === tDate)
  ...
  }, [highlightedAnnouncementId, announcements, chartData, loading, aggregatedDeals])
            ...
            <div className="flex items-center gap-1 bg-zinc-950/50 p-1 rounded-lg border border-zinc-800">
              {(Object.keys(RANGE_MAP) as RangeKey[]).map((range) => (
                <button
                  ...
                  className={`px-2.5 py-1 text-[10px] font-bold rounded-md transition-all ${activeRange === range
